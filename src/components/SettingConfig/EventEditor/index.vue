<template>
    <div class="eventEditor__wrapper" v-if="currentComponent">
        <div>
            <FormItem v-for="(item,index) in finalEventConfig" :key="index" :model-value="eventModel[item.key]"
                @update:model-value="updateEventValue" :form-tag="item.type" :options="item.options" :title="item.title"
                :formKey="item.key" :propAndEvent="item.props">
                {{item.slot}}
            </FormItem>
        </div>
...
import { eventConfig } from '~/constants/config/eventConfig'
